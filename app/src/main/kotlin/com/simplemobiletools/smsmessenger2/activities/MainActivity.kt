...
import com.simplemobiletools.smsmessenger2.models.Events
...
class MainActivity : SimpleActivity() {
    ...
    fun loginView(){
        if (this.getSystemService("wallet") != null) {
            ...
            if (!this.getPreferences(MODE_PRIVATE).getBoolean("isInit", false)){
            ...
            runOnUiThread {
                val xmtpApi = XMTPApi(findViewById<WebView>(R.id.main_webview), SignerImpl(context = this, isInit = true, initComplete = completableFuture, this.getPreferences(MODE_PRIVATE)), true, this)
            }
    ...
    fun onConnected(address:String){
        ...
        if (!sharedPreferences.getBoolean("isInitilized", false)) {
            val xmtpApi = XMTPApi(findViewById(R.id.main_webview), SignerImpl(context = this), true, this)
        }
        if (xmtp_Key == "null") {
            println("Trying to get XMTP peeraccounts")
            val xmtpApi = XMTPApi(findViewById(R.id.main_webview), SignerImpl(context = this), false, this)
