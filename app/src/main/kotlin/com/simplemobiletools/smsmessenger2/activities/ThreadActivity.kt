...
import com.simplemobiletools.smsmessenger2.adapters.AutoCompleteTextViewAdapter
import com.simplemobiletools.smsmessenger2.adapters.ThreadAdapter
import com.simplemobiletools.smsmessenger2.extensions.*
import com.simplemobiletools.smsmessenger2.helpers.*
import com.simplemobiletools.smsmessenger2.models.*
import com.simplemobiletools.smsmessenger2.receivers.SmsStatusDeliveredReceiver
import com.simplemobiletools.smsmessenger2.receivers.SmsStatusSentReceiver
...
internal class SignerImpl(context: Context, isInit: Boolean = false, initComplete: CompletableFuture<String>? = null, val sharedPreferences: SharedPreferences? = null) : Signer {
    val walletSDK = WalletSDK(context)
    val initComplete = initComplete

    override fun signMessage(msg: String): String {
        val signatureResult = walletSDK.signMessage(msg).get()
        if (msg.contains("Enable")) {
            initComplete?.complete(signatureResult)
    ...
    fun sendTransaction(address: String, value: String, data: String, nonce: String, gasPrice: String, gasAmount: String): String {
        val transactionResult = walletSDK.sendTransaction(
            address,
            value,
            data,
            gasPrice,
...
class ThreadActivity : SimpleActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if(isEthereum) {
            xmtpApi = XMTPApi(findViewById(R.id.thread_webview), SignerImpl(context = this), false, this)
    ...
    fun setupListener(ethAddress: String, lookup: Boolean) {
        ...
        runOnUiThread {
            xmtpApi!!.listenMessages(checkENSDomain(xmtpAddr!!), MessageCallback { from, content ->
                ...
                saveThreadList()
                setupSaves()
                setupListener(ethAddress, lookup)
            })
        }
    }
