...
class SimpleContactsHelper(val context: Context) {
    ...
    fun getAvailableContacts(favoritesOnly: Boolean, callback: (ArrayList<SimpleContact>) -> Unit) {
        ensureBackgroundThread {
            ...
            allContacts.forEach {
                ...
                val ethAddress = contact?.ethAddress
                if (name != null) {
                    it.name = name
                }

                if (ethAddress != null && ethAddress != "0x0") {
                    it.ethAddress = ethAddress
                }
    ...
    private fun getContactNames(favoritesOnly: Boolean): List<SimpleContact> {
        ...
        context.queryCursor(uri, projection, selection, selectionArgs) { cursor ->
            ...
            var ethAddress = cursor.getStringValue(Data.DATA15)
            if (isPerson) {
            ...
            if (isOrganization) {
                ...
                if (company.isNotEmpty() || jobTitle.isNotEmpty()) {
                    ...
                    val contact = SimpleContact(rawId, contactId, fullName, photoUri, ArrayList(), ArrayList(), ArrayList(), "0x0")
                    contacts.add(contact)
                }
            }
        }
